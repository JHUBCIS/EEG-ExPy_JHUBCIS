...
class SSVEPExperiment:
    ...
    def __init__(self, frame_rates, positions, hints, trial_len=4, trials_per_block=5, n_blocks=10,
                 screen_refresh_rate=60, IP = "127.0.0.1", Port = 1000):
        """
        Args:
            frame_rates (list): List of number of frames in which each target is flickering (one number for each target)
            positions (list): List of target positions in the screen
    ...
    def run(self):
        ...
        for block_idx in range(self._n_blocks):
            ...
            for trial_idx in range(self._trials_per_block):
                ...
                self._socket.sendto(str(stim_idx + 1).encode(), self._endPoint)
                start_time = time.time()
                ...
                self._socket.sendto(b"end", self._endPoint)
                self.win.flip()
                self.win.flip()
                time.sleep(2)
        self.win.close()
