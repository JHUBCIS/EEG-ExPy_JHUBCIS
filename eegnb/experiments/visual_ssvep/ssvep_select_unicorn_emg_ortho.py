...
class VisualSSVEP_select_unicorn_emg_ortho(Experiment.BaseExperiment):
    '''
    ...
    def present_stimulus(self):
        ...
        emg_prompt = visual.TextStim(
            self.window, 
            text=(
                "Please now generate EMG data:\n"
                "choose a muscle to contract, and press the matching button\n"
                "plase allow a brief pause between contractions"
                ), 
            wrapWidth=30,
            alignText='center',
            color='white')
        ...
        pause_prompt = visual.TextStim(
            ...
            text=(
                ...
                "Prepare for next trial, starting with SSVEP\n"
                ), 
            wrapWidth=30,
            alignText='center',
            color='white')
        ...
        while self.running and (time()-start_time) <= self.record_duration:
            ...
            # present eeg for 10 seconds
            emg_trial_start_time = time()
            while self.running and time() < (emg_trial_start_time + 10): # present eeg for 10 seconds
                emg_prompt.draw()
                self.window.flip()
